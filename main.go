...
func main() {
	...
	// ADD AUTH MIDLLEWARE
	r.Handle("/get/module/{code}", mw.StaffOnly(hand.GetForModule(getModuleDetails))).Methods("GET")
	r.Handle("/get/module/students/{code}", mw.StaffOnly(hand.GetForCode(getModuleStudents))).Methods("GET")
	r.Handle("/get/module/staff/{code}", mw.StaffOnly(hand.GetForCode(getModuleStaff))).Methods("GET")
	r.Handle("/get/cwk/{code}", mw.StaffOnly(hand.GetForCode(getCourseworkDetails))).Methods("GET")
	r.Handle("/get/cwk/students/{code}", mw.StaffOnly(hand.GetForCode(getStudentsOnCwk))).Methods("GET")
	r.Handle("/search/{query}", mw.StaffOnly(hand.GetSearch(search))).Methods("GET")

	r.Handle("/remove/prerequisite", mw.AdminOnly(hand.Update(&dbc.RemovePrerequisite{}, update))).Methods("POST")
	r.Handle("/remove/module/staff", mw.AdminOnly(hand.Update(&dbc.RemoveTeachingStaff{}, update))).Methods("POST")
	r.Handle("/remove/module/student", mw.AdminOnly(hand.Update(&dbc.RemoveStudentModule{}, update))).Methods("POST")

	r.Handle("/update/cwk/results", mw.StaffOnly(hand.Update(&dbc.CwkResult{}, update))).Methods("POST")
	r.Handle("/update/exam/percentage", mw.AdminOnly(hand.Update(&dbc.ExamPercent{}, update))).Methods("POST")
	r.Handle("/update/cwk/percentage", mw.AdminOnly(hand.Update(&dbc.CwkMarks{}, update))).Methods("POST")
	r.Handle("/update/module", mw.StaffOnly(hand.Update(&dbc.EditModule{}, update))).Methods("POST")
	r.Handle("/update/coursework/timetable", mw.StaffOnly(hand.Update(&dbc.UpdateCwkTimetable{}, update))).Methods("POST")

	r.Handle("/add/prerequisite", mw.AdminOnly(hand.Update(&dbc.AddPrerequsite{}, update))).Methods("POST")
	r.Handle("/add/module", mw.AdminOnly(hand.Create(&dbc.NewModule{}, create))).Methods("POST")
	r.Handle("/add/module/student", mw.AdminOnly(hand.Update(&dbc.AddStudentModule{}, update))).Methods("POST")
	r.Handle("/add/staff/tutee", mw.AdminOnly(hand.Update(&dbc.AddTutee{}, update))).Methods("POST")
	r.Handle("/add/module/staff", mw.AdminOnly(hand.Update(&dbc.AddTeachingStaff{}, update))).Methods("POST")
